...
import tevent.events.model.{OfflineParticipant, OnlineParticipant}
...
object EventsServiceTest extends DefaultRunnableSpec {
  ...
  override def spec: ZSpec[TestEnvironment, Any] = suite("Events")(
    ...
    testM("create event and notify if has rights") {
      ...
      val participants = OrganizationParticipantsMock.CheckUser(equalTo((user.id, event.organizationId, OrgManager)), Expectation.unit)
      val service = participants ++ repository ++ notification >>> Events.live
      for {
        result <- Events.create(user.id, event.copy(id = -1)).provideSomeLayer(service)
